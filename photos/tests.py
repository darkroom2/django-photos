...
class LoadBatchCommandTest(TestCase):
    ...
    def test_file_valid(self):  # TODO: add mocking file write
        out = StringIO()
    ...
    def test_file_valid_wrong_format(self):  # TODO: add mocking file write
        out = StringIO()
        call_command('load_batch', 'test_data/photos_invalid.json', '--file', stdout=out)
    ...
    def test_file_parsed(self):
        out = StringIO()
        with self.assertRaises(FileNotFoundError):
            call_command('load_batch', 'fake/path/to/file.json', '--file', stdout=out)
        message = 'Loading photos from file: fake/path/to/file.json'
        self.assertIn(message, out.getvalue())

    def test_url_parsed(self):
        out = StringIO()
        with self.assertRaises(FileNotFoundError):
            call_command('load_batch', 'https://example.com/', '--url', stdout=out)
        message = 'Loading photos from url: https://example.com/'
        self.assertIn(message, out.getvalue())
    ...
    def test_url_valid_wrong_format(self):  # TODO: add mocking file write
        out = StringIO()
        call_command('load_batch', 'https://jsonplaceholder.typicode.com/posts', '--url', stdout=out)
        message = 'Successfully loaded photos: 0, skipped: 100'
        self.assertIn(message, out.getvalue())
